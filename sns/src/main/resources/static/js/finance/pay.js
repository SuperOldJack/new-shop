...
function relatedDoc(that) {
	...
	window.open("finance/queryInLibDocInfo?Code=" + code, 'newwindow', 'left=150,top=100,height=600, width=800');
}
...
function detailed(that) {
	...
	window.open("finance/queryPayDocInfo?Code=" + code, 'newwindow', 'left=150,top=100,height=600, width=800');
}
		...
		"success":function(data) {
			console.log(data);
			 ...
			 for(var i = 0; i < payList.length; i++) {
				  ...
				  payHtml += "&nbsp;&nbsp;&nbsp;<button class='btn btn-primary' onclick='detailed(this)'>查看详情</button></td>";
				  payHtml += "<td>" + payList[i].payment.actual_money + "</td></tr>";
			 }
